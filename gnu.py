...
class segmentation:
    ...
    def __init__(self, packet_type, source, destination, session, udp_data):
        self.session = session
        self.packet_type = packet_type
        self.source = source
...
class start(object):
    ...
    def process(self):
        if self.choice == 1:
            ...
            choices.private_chat(self.sock,destId,msg)
            return True
        elif self.choice == 2:
            choices.group_chat(self.sock,connxn)
            return True
        elif self.choice == 3:
            destId = input("enter the partner address:")
            fileName = input("enter the filename:")
            choices.file_transfer(self.sock,destId,fileName)
            return True
        elif self.choice == 4:
